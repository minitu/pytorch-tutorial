...
class Worker:
  def __init__(self, args):
    ...
    if self.dist.is_cpu_rank():
      ...
      print("[Rank {}] Setting number of OMP threads to {}".format(self.rank,
        args.num_threads), flush=True)
  ...
  def train(self, args):
    ...
    data_loader = get_loader(args.image_dir, args.caption_path, vocab,
                             ...
                             self.gpu_batch_size, self.batch_size,
      ...
      for i, (images, captions, lengths) in enumerate(data_loader):
        ...
        if i % args.log_step == 0 and i != 0:
          print('Rank [{}], Epoch [{}/{}], Step [{}/{}], Average time: {:.6f}, Loss: {:.4f}, Perplexity: {:5.4f}'
                  .format(self.rank, epoch, args.num_epochs, i, total_step, batch_time_sum / processed_batches, saved_loss, np.exp(saved_loss)), flush=True)
          batch_time_sum = 0
          processed_batches = 0
...
def main():
  ...
  parser.add_argument('-r', '--reproducible', action='store_true', default=False, help="make runs reproducible")
